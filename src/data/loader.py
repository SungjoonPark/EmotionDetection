...
class ISEARLoader(EmotionDatasetLoader):
    ...
    def load_data(self):
        ...
        for s_name, s_data in zip(self.split_names, splits):
            ...
            count_list = self.count_label(labels, count_list)
            data[s_name] = {}
            for name, d in zip(self.data_types, [text, labels]):
                data[s_name][name] = d
        print(count_list)
...
class GOEMOTIONSEkmanLoader(EmotionDatasetLoader):
    ...
    def __init__(self):
        ...
        self.path = os.path.join(self.rel_path, "./../../data/ekman/")
...
def main():
    ...
    print(goemotions.get_vad_coordinates_of_labels())
    # print(len(data['test']['label']))

    print("---- EKMAN ----")
    ekman = GOEMOTIONSEkmanLoader()
    data = ekman.load_data()
    ekman.check_number_of_data()
    print(ekman.get_vad_coordinates_of_labels())
